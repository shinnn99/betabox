 ...
 *
 * Tách khỏi route /api/warehouse/live/proof-size-risk để trang Giám sát
 * hoàn hàng (route riêng /api/returns/live/proof-size-risk) dùng lại được
 * mà không chép 200 dòng ước lượng thành hai bản chực lệch nhau. Clip kiện
 * hoàn dài tới 5 phút nên càng dễ vượt trần tải lên hơn đơn đi.
 */

export const PROOF_RISK_DEFAULT_LIMIT = 60;
export const PROOF_RISK_MAX_LIMIT = 200;
...
export interface ProofSizeRiskResult {
  risks: RiskRow[];
  upload_guard_bytes: number;
  warn_bytes: number;
}

export function parseProofRiskLimit(raw: string | null): number {
  const n = raw ? Number.parseInt(raw, 10) : PROOF_RISK_DEFAULT_LIMIT;
  if (!Number.isFinite(n) || n <= 0) return PROOF_RISK_DEFAULT_LIMIT;
  return Math.min(n, PROOF_RISK_MAX_LIMIT);
}

/** Throw khi query gốc lỗi — route quyết cách trả lỗi. */
export async function buildProofSizeRisks(params: {
  admin: Admin;
  orgId: string;
  day: { startIso: string; endIso: string };
  limit: number;
  eventKind: "outbound" | "return";
}): Promise<ProofSizeRiskResult> {
  const { admin, orgId, day, limit, eventKind } = params;
  ...
  const { data: events, error: evErr } = await admin
    ...
    // không lọc loại nên kiện hoàn lọt vào ước lượng của trang đóng hàng.
    .eq("event_kind", eventKind)
    .in("status", eventKind === "return" ? ["valid", "return_suspect"] : ["valid"])
