...
/**
 * Đóng kiện hoàn đang mở của một bàn bằng thẻ kết quả (hoặc thẻ KẾT THÚC).
 *
 * Không có kiện nào đang mở thì trả `closed: false` — tầng gọi báo cho nhân
 * viên biết, thay vì im lặng như chưa từng quét.
 */
export async function closeOpenReturnWithResult(params: {
  admin: Admin;
  organizationId: string;
  stationId: string;
  result: InspectionResult;
  closeReason: "result_card" | "end_card";
  at: string;
}): Promise<CloseReturnOutcome> {
  const { data: open } = await params.admin
    .from("packing_events")
  ...
  if (!open) return { closed: false, event_id: null, waybill_code: null, result: null };

  const { data: closed, error } = await params.admin.rpc("close_return_event", {
    p_event_id: open.id,
  ...
  if (error) {
    console.error(`[return-scan] close_return_event lỗi event=${open.id}: ${error.message}`);
    return { closed: false, event_id: open.id as string, waybill_code: null, result: null };
  }

  return {
    closed: closed === true,
    event_id: open.id as string,
    waybill_code: (open.waybill_code as string) ?? null,
