...
export function parseActivityLimit(raw: string | null): number {
  const n = raw ? Number.parseInt(raw, 10) : ACTIVITY_DEFAULT_LIMIT;
...
export async function buildLiveActivity(
  ...
  if (rawIds.length === 0) return { activity: [], ...meta };
  ...
  if (orphanRaws.length > 0) {
    ...
    const stationIds = Array.from(
      new Set(results.map((x) => x.stationId).filter((x): x is string => !!x)),
    );
    if (stationIds.length > 0) {
      ...
      for (const r of results) {
        ...
        const stationMeta = byId.get(r.stationId);
        if (stationMeta) {
          orphanResolved.set(r.rawId, {
            station_code: stationMeta.code,
            station_name: stationMeta.name,
            warehouse_code: stationMeta.warehouse_code,
          });
        }
      }
    }
