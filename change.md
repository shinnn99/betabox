...
- **Files test:** `tests/return-dot7-nhat-ky-bao-cao.test.ts` (mới, 9 bài); cập nhật `tests/return-pages.test.ts` và `tests/counting-excludes-returns.test.ts` theo luật mới.
- **Kết quả kiểm tra:** `pnpm test` 543/543, `tsc` đạt, `eslint` không lỗi. Đã áp cả hai migration và kiểm trên web thật (org test KHO_HN_01): lưu 10 ngày → 200 và đọc lại đúng; nhập 3 ngày bị chặn 400; máy kho nhận số ngày mới qua `/api/agent/retention-plan`; báo cáo trả khung hàng hoàn; sản lượng đóng hàng không đổi.
- **Trạng thái:** Hoàn tất.
