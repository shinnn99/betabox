...
- **Files tạo/sửa:** `warehouse-agent/releases/BetacomAgentSetup-v0.12.0.exe` (mới), `change.md`.
- **Chi tiết thay đổi:** Không sửa mã nguồn. `package.json` và `installer/betacom-agent.iss` đã sẵn ở `0.12.0`, `AppURL` đã là `https://betabox.betacom.agency` (không dính domain cũ). Chạy `npm run build:exe` rồi `ISCC.exe installer/betacom-agent.iss`, chép kết quả từ `dist-installer/` sang `releases/`.
- **Kết quả kiểm tra:** Kiểm đủ 6 file phụ thuộc trước khi đóng gói (ffmpeg, ffprobe, nssm, mediamtx, VERSION.txt, cleanup-segments.ps1) — đều có thật, header `MZ`. Compile thành công 263,8 s, ra file 155.156.477 bytes (~147 MB), khớp cỡ với bộ cài 0.11.0 (147 MB). SHA256 `2fb2740a8dcfd97a` giữ nguyên sau khi chép sang `releases/`. Đối chiếu `git diff 9efa721 HEAD -- warehouse-agent/src/` rỗng: mã nguồn không đổi từ lúc phát hành file trần 0.12.0, nên hash file chạy lệch (`e8c3addc` vs `f70b3b5e`) chỉ do `pkg` build không tất định, không phải khác code.
...
### [TIEUCHI-CAMERA] - Bổ sung loại camera + tiêu cự vào bộ tiêu chí, và sửa ba chỗ đã lỗi thời

- **Mục tiêu:** Chủ dự án hỏi tiêu chí camera: loại USB hay IP/PoE, thông số, tiêu cự, chuẩn video hệ thực nhận. File `plans/reports/TIEU-CHI-THONG-SO-TOI-THIEU-2026-09-23.md` thiếu hai mục đầu và có ba chỗ đã lệch với code sau agent 0.11.0.
- **Thêm §0b "Loại camera":** khẳng định **IP/PoE, không hỗ trợ USB** — đã kiểm bằng grep toàn repo, không có `dshow`/`v4l2`/`avfoundation` ở đâu; mọi đường vào đều là RTSP (`recording.ts:287`, `qr-frame-source.ts:114`). Các kết quả "USB" trong mã nguồn đều là máy quét mã vạch COM/USB-serial, không phải camera. Kèm bảng chuẩn video hệ nhận và ghi rõ **ghi hình dùng `-c:v copy` nên camera đặt sao thì ổ nhận vậy** — đây là gốc của việc phải chốt thông số TRONG camera.
- **Thêm tiêu cự (mục file chưa từng có):** toàn cảnh **2.8mm** (góc ~100°, phủ bàn 1,2–1,5 m ở độ cao 2,2–2,5 m); quét mã **4mm** (khung hẹp, chỉ ôm một nhãn ở cự ly 40–50 cm). Suy từ ràng buộc đã có trong file (góc đặt, khoảng cách, "chỉ thấy 1 nhãn"), không phải khuyến nghị chung của hãng.
- **Sửa ba chỗ đã lỗi thời/tả sai:**
  - **Độ phân giải camera quét mã:** file ghi lý do "khung bị hạ về 640×360 xám" — agent 0.11.0 (`2a57b63`) đã bỏ mức ép cứng đó, giờ ffprobe độ phân giải thật và đọc ở đúng cỡ, **trần 2560×1440** (`qr-frame-source.ts:19-23`). Hệ quả đổi chiều kết luận mua sắm: camera nét hơn **giờ có giá trị thật**, khuyến nghị nâng lên 1080p–2K.
  - **Luật nhiều mã trong khung:** file tả "thấy 2 mã → bỏ cả hai". Sai. `code-pick.ts:77-104` cho **nội dung xuất hiện nhiều lần nhất thắng** (nhãn vận đơn in mã lặp lại); chỉ `ambiguous` khi hai nội dung KHÁC nhau mà bằng số lần và to xấp xỉ (< 1,5×) — dấu hiệu hai nhãn.
  - **"2 frame liên tiếp":** bộ đếm reset khi khung cho ra mã khác, không đòi liên tiếp tuyệt đối (`qr-zone.ts:73-87`). Đã tả lại.
- **Thêm mã vạch một chiều vào tiêu chí:** agent 0.11.0 đọc thêm `Code128, Code39, Code93, ITF, Codabar` ngoài QR/DataMatrix (`qr-decoder.ts:71-72`); cố ý **không** mở EAN/UPC vì đó là mã sản phẩm trên hộp, đọc trúng là tạo đơn sai. File trước chỉ nói về QR.
- **Nặng hơn ở hàng Codec:** ngoài "clip không phát được trên `<video>`", bổ sung hậu quả thật: codec guard lúc cắt clip có thể làm **clip failed — không ra file bằng chứng nào** (`codec-invalidation.ts:3-5`).
- **Thêm §8 tóm tắt mua sắm:** bảng đối chiếu hai camera, ba câu phải hỏi người bán (ép được H.264 không / xuất được hình dọc không / có sub-stream không), và danh sách "đừng mua" (4K, varifocal, Wi-Fi, webcam USB).
- **Kết quả kiểm tra:** chỉ sửa tài liệu, không đụng mã nguồn nên không chạy test. Mọi số và mọi trích dẫn dòng trong phần thêm/sửa đều đọc thẳng từ file nguồn tại thời điểm sửa.
- **Trạng thái:** Hoàn tất.
...
- **Sau khi áp 20260924100000 — phát hiện thêm một lỗi do chính migration đó:** `CREATE OR REPLACE FUNCTION` chỉ thay khi TRÙNG danh sách tham số, nên thêm `p_force` sinh ra bản hàm THỨ HAI bên cạnh bản cũ 4 tham số. Từ đó mọi lời gọi 4 tham số đều chết với `could not choose the best candidate function`:
  - tắt phiên hoàn THƯỜNG (không ép) hỏng hẳn — đúng cái vừa đi sửa;
  - `expire_return_captures` gọi hàm này bên trong SQL cũng chết (`function ... is not unique`), tức lưới an toàn dọn người giữ treo sau 2 phút mất luôn. Đo trực tiếp: dựng một kỳ hoàn mất nhịp 5 phút, gọi hàm hết hạn → lỗi, kỳ vẫn mở với người giữ treo.
  - Sửa: `supabase/migrations/20260924110000_drop_old_release_signature.sql` (mới, CHƯA ÁP) bỏ bản 4 tham số; mã nguồn LUÔN truyền `p_force` để khớp đúng một bản.
- **Giao ca giữa hai luồng khi đoạn video đang ghi dở** (chủ dự án nhắc 24/09/2026): agent vốn đã làm đúng — đoạn dở của luồng cũ giữ nguyên chủ tới khi đóng, luồng mới tính từ đoạn kế tiếp, và phiên cũ chỉ báo xong sau khi đoạn cuối lưu xong. Khoá lại bằng `warehouse-agent/tests/mode-switch-overlap.test.ts` (4 bài, gồm 6 vòng chuyển qua lại khi lúc nào cũng có đoạn đang ghi dở).
...
- **Triệu chứng:** chủ dự án báo 25/09/2026 — bấm Xoá ở trang Người dùng hệ thống hiện toast đỏ chỉ có dấu `{}`, không đọc được lỗi gì.
- **Nguyên nhân 1 — khoá ngoại chặn xoá:** `auth.admin.deleteUser()` xoá dòng trong `auth.users`, nhưng còn **hai** bảng trỏ vào `auth.users` với `NO ACTION` nên Postgres chặn cứng: `camera_recording_sessions.created_by` và `order_proof_clips.generated_by`. Mọi khoá ngoại khác (`audit_logs`, `staff_profiles`, `return_claims`, `order_proof_requests`…) đã là SET NULL hoặc CASCADE — hai bảng này là ngoại lệ sót lại, không phải chủ ý giữ vết. Đếm trên database thật: 3 tài khoản vướng (Betacom 7 phiên, Nguyễn Hà Giang 5, Nguyễn Đình Hạnh 1), 5 tài khoản còn lại 0 phiên nên xoá vẫn chạy — khớp việc lỗi xuất hiện không đều.
- **Nguyên nhân 2 — vì sao ra ô rỗng:** `message` của `Error` là **non-enumerable**, nên khi `AuthError` đi qua `JSON.stringify` nó rụng mất, body tới trình duyệt chỉ còn `{name, status, code}`. Client làm `data.message ?? data.error` — vế đầu `undefined`, rơi sang `data.error` vốn là **object**, mà `??` chấp nhận object → object lọt vào Toast (khai báo `message: string`) → React in ra `{}`. Lỗi thật bị nuốt sạch.
- **Sửa:**
  - `supabase/migrations/20260925100000_user_delete_set_null_fks.sql` (mới, ĐÃ ÁP 25/09/2026): đổi hai khoá ngoại sang `ON DELETE SET NULL`. Chủ dự án chốt hướng này — phiên ghi hình và clip bằng chứng **giữ nguyên**, chỉ mất thông tin "ai đã bật", đúng mức đánh đổi `audit_logs` đã chọn. Hai cột vốn đã nullable nên không cần đổi kiểu cột.
  - `src/components/ui/Toast.tsx`: thêm `toText()` chuẩn hoá mọi giá trị trước khi hiện, đặt ở **cửa chung** thay vì vá `??` ở từng trang rồi lần sau lại sót. Object có `message`/`error` dạng chuỗi thì moi ra dùng; còn lại ghi Console và hiện câu hướng dẫn. `error()` đổi sang nhận `unknown` có chủ đích — ép kiểu `string` chỉ làm TypeScript im lặng chứ không chặn được object lúc chạy.
  - `src/app/api/users/[id]/route.ts`: bọc `try/catch` quanh phần xoá (trước đây `deleteUser` throw thì Next trả 500 với body khác hợp đồng, cũng ra ô rỗng). Bắt riêng vi phạm khoá ngoại → 409 kèm câu tiếng Việt "Không xoá được tài khoản vì còn dữ liệu đang tham chiếu tới. Hãy khoá tài khoản thay vì xoá, và báo kỹ thuật." Đọc `err.message` ra chuỗi TRƯỚC khi đưa vào JSON.
- **Kết quả kiểm tra — đo cả hai vế trên database thật:**
  - Vế dương: dựng user giả + 1 phiên ghi hình trỏ vào nó → xoá **thành công**, và phiên ghi hình **vẫn còn** với `created_by = NULL` (0 dòng trỏ tới user đã xoá). Chạy trong transaction rồi ROLLBACK, không đụng dữ liệu thật.
  - Vế âm: tạm dựng lại khoá ngoại `NO ACTION` như trước migration → xoá **bị chặn** đúng bằng `foreign_key_violation`, chứng minh lỗi có thật chứ không phải đoán. Kiểm lại sau khi rollback: khoá ngoại vẫn nguyên `SET NULL`, không để lại tác dụng phụ.
  - `toText` chạy qua 5 ca gồm đúng payload gây lỗi (`AuthError` sau `JSON.stringify`): không ca nào còn ra `{}` hay `[object Object]`.
  - `tsc` đạt (không chỗ gọi `toast.error` nào vỡ), `eslint` hai file đã sửa không lỗi.
- **Còn lại:** chủ dự án bấm Xoá trên web thật để xác nhận đường đi trọn vẹn từ giao diện.
- **Trạng thái:** Hoàn tất phần code và database, chờ kiểm trên web thật.

### [XOA-THIET-BI] - Xoá thiết bị kho: báo xoá xong mà vẫn còn, và một nút xoá thật không ai chắn

- **Triệu chứng:** chủ dự án báo 25/09/2026 — xoá `qrcam_cqr01` ở trang Thiết bị kho, toast báo "Đã xoá" nhưng dòng vẫn nằm đó, cột Cập nhật ghi "0s trước".
- **Bằng chứng khít từng giây:** `audit_logs` ghi `station_device.archive` lúc `01:17:39.914`, `station_devices.updated_at` của chính dòng đó là `01:17:40.270` — bị ghi lại sau **356ms**. `created_at` vẫn là 22/09, tức cùng một dòng bị bật lại chứ không phải tạo dòng mới. Đã thử 2 lần (01:03 và 01:17), lần nào cũng vậy.
- **Nguyên nhân:** nút Xoá máy quét là **xoá mềm** (`status: "archived"`, quyền tên `station_device.archive`). Ngay sau đó bộ tự-sửa `planVirtualScannerRepairs` thấy camera `CQR01` vẫn gán `BAN_01` và bàn đó `scan_source = 'camera'` → sinh việc `activate` → ghi lại `status: "active"`.
- **Nguồn gốc máy quét ảo (truy theo yêu cầu):** không phải chắp vá, là thiết kế có chủ đích của dự án 2-camera, commit `a696146` ngày 14/09/2026, ghi rõ trong `docs/tuan-tu-xu-ly-2-camera.md` bước 5 và `plans/active/B.2-cloud-camera-station-api.md`. Lý do: agent báo mã đọc từ camera dưới tên `qrcam_<mã camera>`; cloud tra tên đó qua `resolve_scanner_at` — đã đọc định nghĩa thật trên database, hàm đòi `d.status = 'active'` VÀ có phân công đang mở. Thiếu một trong hai thì mọi lần quét trả `unmapped_scanner`: mã vẫn lưu thô nhưng không quy về bàn, không tạo sự kiện đóng đơn, không có video bằng chứng. Dùng bản ghi camera sẵn có không được vì một camera đồng thời là nguồn quay (`device_type='camera'`) và nguồn đọc mã (`device_type='scanner'`) — một dòng không mang được hai `device_type`. Bộ tự-sửa thêm sau vì sự cố 16/09: camera `dahua_3` chuyển bàn không qua đường chuẩn, scanner ảo bị archive, mọi lần quét hỏng trong im lặng hơn một ngày.
- **Rà soát phát hiện lỗ nặng hơn ở nút bên cạnh:** nút Xoá camera là **xoá THẬT**. Khoá ngoại `order_proof_clips.camera_id` là RESTRICT nên chặn được camera đã cắt clip, nhưng `camera_recording_files` và `camera_recording_sessions` là **CASCADE**. Đơn chưa ai bấm xem clip thì không có dòng clip nào nên **không gì chặn**. Đo trên database thật: `CQR01` có 0 clip → xoá được ngay, kéo theo **1.122 bản ghi đoạn video (21 GB)** và **230 đơn còn trong hạn** mất đường cắt clip. File .mp4 trên ổ máy kho vẫn còn (bản ghi DB và file trên đĩa là hai thứ độc lập — `src/app/api/agent/retention-plan/route.ts` dòng 96-98) nhưng không còn gì trỏ tới, thành video mồ côi chiếm ổ. Hộp xác nhận cũ chỉ nói "Lịch sử recording liên tục cũng sẽ bị xoá".
- **Sửa:**
  - `src/lib/camera/service.ts`: thêm `HasRecentOrdersError` và lưới thứ hai trong `deleteCamera` — đếm `packing_events` trong hạn `organizations.retention_days` trỏ tới camera (cả `proof_camera_id` lẫn `proof_qr_camera_id`), có thì chặn. `retention_days` NULL hoặc ≤ 0 thì KHÔNG chặn theo đơn: không có cửa sổ thì không tự bịa số, để lưới clip gánh.
  - `src/app/api/cameras/[id]/route.ts`: trả 409 kèm `orders_count`, `files_count`, `retention_days` và câu tiếng Việt chỉ đường "chuyển sang trạng thái Ngừng thay vì xoá".
  - `src/app/dashboard/devices/page.tsx`: nút và hộp xác nhận của máy quét đổi "Xoá" → **"Lưu trữ"** cho khớp việc thật sự xảy ra (trang `DevicesTab` vốn đã gọi đúng là "Lưu trữ" — hai trang đang nói hai tên khác nhau cho cùng một hành động). Máy quét ảo có hộp thoại riêng giải thích nó không phải thiết bị thật và sẽ tự bật lại nếu bàn còn quét bằng camera; máy quét thật đang gán bàn thì cảnh báo bàn sẽ không nhận mã nữa.
- **Files test:** `tests/camera-delete-guard.test.ts` (mới) — 8 bài: ca CQR01 thật, lưới clip vẫn chạy trước, camera sạch vẫn xoá được, đơn quá hạn không giữ camera lại, chưa cấu hình retention thì không chặn, và nhận diện máy quét ảo không nhầm với súng quét thật.
- **Kết quả kiểm tra:** `pnpm test` 566/566, `tsc` đạt, `eslint` không lỗi mới. Mô phỏng đúng hai truy vấn của lưới trên dữ liệu thật: `CQR01` và `dahua_3` chuyển từ "xoá được" sang **bị chặn**, `CAM_TEST` (0 clip, 0 đơn) vẫn **cho xoá** — lưới không chặn bừa. Kiểm riêng cú pháp `.or()`: CQR01 có 0 đơn ở `proof_camera_id` nhưng 230 đơn ở `proof_qr_camera_id`, nên thiếu vế OR là bỏ lọt toàn bộ.
- **Rà soát thêm, chưa sửa:** quét toàn bộ bàn `scan_source = 'camera'` — hiện **không bàn nào đang hỏng**, cả 2 camera đọc mã (`CQR01`→BAN_01, `dahua_3`→BAN_04) đều có scanner ảo `active` và đã gán bàn. Còn tồn: `hik_3` có 2 bản ghi camera trùng mã, bản `auto_hik_3_oq78` không vai trò, không gán bàn — rác từ lần tạo trùng trước.
- **Trạng thái:** Hoàn tất.

### [QUET-KHONG-RO-BAN] - Chặn lưu trữ máy quét ảo đang dùng, và bịt chỗ mù "hỏng trong im lặng"

- **Câu hỏi khơi ra:** chủ dự án hỏi "lưu trữ máy quét ảo đó có ảnh hưởng gì hệ thống không". Truy đường quét thật thì câu trả lời là **có**, và nặng hơn tưởng.
- **Chuỗi hậu quả, đọc từ định nghĩa thật trên database:** `resolve_scanner_at` đòi thiết bị `status='active'` VÀ có phân công đang mở → archived thì trả rỗng → `process_waybill_scan` đặt `v_status := 'unmapped_scanner'` → dòng `packing_events` vẫn ghi nhưng **ép NULL cả `station_id`, `warehouse_id` lẫn `proof_camera_id`**, không tạo `orders`, không gắn ca, `timing_status = 'not_applicable'`. Đơn không được đếm, không vào báo cáo năng suất, không có video bằng chứng.
- **Điều làm nó nguy hiểm:** bộ tự-sửa `planVirtualScannerRepairs` **chỉ chạy trong `listCameras()`**, tức đường đọc dashboard. Không ai mở trang thì bản ghi nằm archived vô thời hạn. Bấm lưu trữ rồi đóng trình duyệt, sáng hôm sau kho quét trước khi ai kịp mở dashboard → toàn bộ lượt quét rơi vào `unmapped_scanner`.
- **Đã cắn thật, tìm được trong `packing_events`:** 4 lượt `unmapped_scanner` ngày 16/09/2026, **cả 4 đều mang `scanner_device_code = 'qrcam_dahua_3'`** — chính máy quét ảo. Tất cả mất bàn, mất ca, không tạo đơn. Nhân viên quét lại 3 lần cùng một mã `SPXVN069339340428` vì không thấy phản hồi. Dashboard không báo gì; agent chỉ ghi một dòng log.
- **Sửa 1 — chặn tại API (`src/app/api/station-devices/[id]/route.ts`):** lưu trữ máy quét `qrcam_*` mà bàn đang gán còn đặt `scan_source = 'camera'` → trả **409 `virtual_scanner_in_use`** kèm tên bàn và chỉ đường "chuyển nguồn đọc mã của bàn sang súng quét — hệ thống sẽ tự gỡ máy quét ảo này". Chặn ở API chứ không chỉ ở giao diện, vì cửa sổ hỏng là thật chứ không phải phiền toái hiển thị. Giao diện bỏ câu "hệ thống sẽ tự bật lại" — hứa đó đúng nhưng thiếu, nó giấu mất việc quét sẽ hỏng trong lúc chờ và việc bật lại phụ thuộc có người mở trang.
- **Sửa 2 — mục theo dõi mới `unmapped_scanner` (`src/lib/system/checks.ts`, `status-view.ts`):** đếm lượt quét không quy được về bàn trong 24 giờ, theo từng kho. **Một lượt đã là `warn`** (không có ca lẻ vô hại ở đây), từ 3 lượt là `crit` — đúng dấu vết sự cố 16/09, nhân viên quét lại nhiều lần vì không thấy phản hồi. Entity nêu **tên thiết bị** làm manh mối: `qrcam_*` thì chỉ sang đường máy quét ảo, tên khác thì chỉ sang phân công súng quét thật. Đây là mục thứ 11 trong loạt kiểm, Lark sẽ gửi cảnh báo như các mục khác.
- **Sửa 3 — ẩn nút thay vì mời bấm rồi từ chối (bổ sung sau khi chủ dự án hỏi "sao nó vẫn hiển thị nút lưu trữ?"):** bản đầu chỉ chặn ở API nên nút vẫn hiện, bấm vào mới biết bị từ chối — nút hiện ra là một lời mời, mời xong lại từ chối thì người vận hành tưởng hệ thống hỏng. Nay `virtualScannerLocked()` tra `scan_source` của bàn (đã có sẵn trong `stations` mà trang nạp từ `/api/packing-stations`, đã kiểm API thật sự trả cột này) và rút nút ở CẢ hai chỗ: menu ba chấm và chân hộp chi tiết máy quét. Chỗ nút bị rút hiện dòng giải thích "Bàn đang đọc mã bằng camera nên máy quét ảo này phải giữ. Đổi nguồn đọc mã của bàn sang súng quét thì hệ thống tự gỡ" — menu cụt lủn không lời giải thích cũng đọc thành "hệ thống lỗi". Không tra được bàn thì KHÔNG ẩn: thà để API từ chối còn hơn giấu mất đường thao tác hợp lệ. Chốt thật vẫn ở API, đây chỉ là lớp hiển thị cho khớp.
- **Files test:** `tests/system-checks.test.ts` — 6 bài mới (0 lượt → ok kèm ô "0 lượt" hiện ra; 1 lượt → warn; 3 lượt → crit; tái hiện sự cố 16/09 nêu đúng `qrcam_dahua_3`; súng quét thật không bị chỉ nhầm sang đường máy quét ảo; nguồn hỏng → unknown chứ không xanh giả). Cập nhật bài canh danh sách mục: 10 → 11. `tests/camera-delete-guard.test.ts` — 5 bài cho việc ẩn nút, gồm ba vế âm: bàn đã chuyển về súng quét thì MỞ nút; súng quét thật ở chính bàn dùng camera vẫn lưu trữ được; không tra được bàn thì không khoá.
- **Kết quả kiểm tra:** `pnpm test` **577/577**, `tsc` đạt, `eslint` 0 error.
  - **Vế âm đã đo (hai lần, hai chỗ):** phá logic mức độ của mục theo dõi (ép luôn ra `"ok"`) → đúng 2 bài đỏ, đúng hai bài canh warn/crit. Phá logic ẩn nút (bỏ kiểm tiền tố `qrcam_`) → đúng bài "súng quét thật vẫn lưu trữ được" đỏ, tức lưới sẽ khoá nhầm thiết bị thật. Test bắt được lỗi thật chứ không pass suông. Đã khôi phục cả hai file và chạy lại xanh.
  - **Trên dữ liệu thật:** mô phỏng đúng điều kiện API — `qrcam_cqr01` (BAN_01) và `qrcam_dahua_3` (BAN_04) **bị chặn 409**, còn `MAY_QUET_01` là súng quét thật cùng bàn BAN_01 **vẫn lưu trữ được**. Lưới không chặn bừa. Mục theo dõi mới trả 0 lượt/24h cho cả hai kho — xanh đúng trạng thái thật.
- **Trạng thái:** Hoàn tất.

### [KHOA-TK-KHONG-AN] - Tài khoản "disabled" vẫn đăng nhập được vào hệ thống

- **Triệu chứng:** chủ dự án báo 25/09/2026 — `nguyenhagiang@betacom.vn` hiện `disabled` trên trang Người dùng hệ thống nhưng vẫn đăng nhập vào được.
- **Gốc:** `user_profiles.status` là cột của RIÊNG ứng dụng — Supabase Auth không biết nó tồn tại. Đo trên database thật: `status='disabled'` nhưng `auth.users.banned_until IS NULL`. Grep toàn bộ mã nguồn: **không chỗ nào từng ghi vào `banned_until`** — hai chỗ duy nhất gọi `updateUserById` đều chỉ để đổi mật khẩu. Nên `signInWithPassword` vẫn cấp token như thường; khoá tài khoản lâu nay chỉ là đổi một chữ trong bảng.
- **Vì sao nguy, không phải phiền toái nhỏ:** đếm trong `src/app/api` — **37 route dùng `requirePermission`** (không-strict) đọc vai trò thẳng từ JWT, KHÔNG hỏi lại DB, nên vẫn phục vụ người đã bị khoá; chỉ **32 route dùng `requirePermissionStrict`** mới chặn bằng `account_disabled`. Trong nhóm không-strict có 2 route GHI: `POST /api/warehouse/manual-scan` (tạo đơn) và `POST /api/returns/capture`. Dự án cũng KHÔNG có `middleware.ts`, và `dashboard/layout.tsx` không kiểm `status` — nên không có lớp chặn nào phía trước.
- **Sửa — `src/app/api/users/[id]/route.ts`:**
  - Đổi `status` giờ kéo theo tầng xác thực: `admin.auth.admin.updateUserById(id, { ban_duration })` — `876000h` (100 năm) khi khoá, `none` khi mở. Đã kiểm chữ ký `ban_duration` trong SDK trước khi dùng.
  - Huỷ phiên đang mở khi khoá. GoTrue thu hồi refresh token lúc ban, nhưng access token đã phát vẫn sống tới hết hạn (mặc định 1 giờ) — đúng khoảng đó thì 37 route không-strict vẫn phục vụ vì chỉ đọc JWT.
  - Validate `status`: trước đây nhận thẳng chuỗi bất kỳ rồi ghi vào DB. Gõ nhầm `"disable"` (thiếu chữ d) là tài khoản không bị khoá mà cũng không ai biết.
  - Tầng auth hỏng thì **báo cho admin** (`warning: auth_ban_failed`) chứ không im lặng trả `ok: true` — im lặng nghĩa là admin đóng màn hình với niềm tin đã khoá xong, đúng cảnh vừa xảy ra.
- **Migration `20260925110000_revoke_sessions_on_disable.sql` (ĐÃ ÁP):** hàm `revoke_user_sessions(uuid)` SECURITY DEFINER xoá phiên + refresh token. Cần hàm riêng vì đã đo quyền: trên `auth.sessions` **chỉ vai trò `postgres` có quyền**, `service_role` không có gì — cấp quyền ghi thẳng schema `auth` cho service_role là mở rộng bề mặt tấn công cho mọi đường dùng service key. Đã kiểm `auth.refresh_tokens.user_id` là `varchar` chứ không phải `uuid` nên so theo text, tránh một giá trị rác làm đổ cả lệnh khoá.
- **Vá dữ liệu tồn:** đặt `banned_until` cho tài khoản đã `disabled` mà auth chưa biết — đúng 1 dòng (`nguyenhagiang@betacom.vn`), không đụng ai khác.
- **Files test:** `tests/user-disable-auth-sync.test.ts` (mới) — 7 bài: khoá phải ban ở tầng auth; khoá phải huỷ phiên; mở khoá gỡ ban và KHÔNG đá phiên (đá là phá ngang người đang làm việc); ban đủ dài để không tự hết hạn; chặn giá trị status lạ gồm ca gõ nhầm "disable".
- **Kết quả kiểm tra:** `pnpm test` **583/583**, `tsc` đạt, `eslint` sạch.
  - **Vế âm (lỗ có thật):** trước khi vá, truy vấn xác nhận Giang `status='disabled'` + `banned_until IS NULL` → cửa đăng nhập mở.
  - **Vế dương (RPC chạy thật):** thử trên tài khoản có 3 phiên — `trước=3, xoá=3, sau=0`, rồi ROLLBACK; kiểm lại 3 phiên còn nguyên, không đá nhầm ai.
  - **Quyền đúng cả hai chiều:** `service_role` gọi được hàm, `authenticated` và `anon` **không** — không lộ ra client để người dùng đá phiên của nhau.
  - **Sau khi vá:** rà toàn bộ 8 tài khoản, **0 dòng lệch** — Giang bị chặn đăng nhập, 7 người còn lại không ai bị khoá nhầm.
- **Còn lại:** 2 route GHI dùng guard không-strict (`manual-scan`, `returns/capture`) vẫn chưa chuyển sang Strict. Sau thay đổi này người bị khoá không lấy được token mới và phiên cũ đã bị đá, nên đường khai thác đã đóng; nhưng chuyển nốt cho nhất quán vẫn nên làm.
- **Trạng thái:** Hoàn tất.

### [NHATKY-DOC-HIEU] - Nhật ký hệ thống viết lại cho người vận hành kho đọc

- **Triệu chứng:** chủ dự án nhắc 25/09/2026 — "để nhật ký dạng log như này ở đây thì người dùng sao hiểu". Trang Quản lý hệ thống → Nhật ký hệ thống đổ thẳng dữ liệu thô ra màn hình: cột Đối tượng hiện UUID cắt 8 ký tự (`3f2a1b9c…`), cột Chi tiết hiện `JSON.stringify(metadata)` nguyên xi, và bảng nhãn chỉ dịch được **12** action trong khi hệ đang ghi **43** — action ngoài bảng rơi thẳng ra mã gốc (`camera.delete`, `station_device_assignment.assign`).
- **Vị trí hai trang (trả lời câu hỏi kèm theo):** cả hai đều ở dashboard của từng kho, quyền `audit.view`. Nhưng khác bản chất — Nhật ký hệ thống là dữ liệu **riêng từng tổ chức** (API đi qua `getScopedClient`), còn Nhật ký cập nhật phiên bản là **hằng số dùng chung**, nằm trong dashboard vì người đọc là nhân viên kho. Platform đã có `/platform/audit` nhưng chỉ ghi thao tác của quản trị nền tảng, **không** gộp audit của các org — muốn xem phải impersonate. Chưa làm trang tổng hợp ở platform lần này.
- **Đối chiếu dữ liệu thật trước khi viết (không suy từ mã nguồn):**
  - Quét `audit_logs` thật: có **9 action lịch sử** mà grep mã nguồn KHÔNG bao giờ thấy vì route đã đổi tên hoặc bỏ (`camera.recording.sync_files` 40 dòng, `camera.test_connection` 29, `camera.recording.start` 20, `order_proof.clip.generate` 19…). Dữ liệu cũ vẫn nằm trong bảng và vẫn hiện ra màn hình — bỏ qua là người dùng xem lại lịch sử thấy mã trần. Đã khai nhãn cho cả 9.
  - Quét khoá trong `metadata->changes`: **8 trường** chưa có nhãn, trong đó `config_json` xuất hiện 26 lần là khối JSON lớn — đổ nguyên ra bảng thì chiếm hết màn hình. Cho hiện "(đã thay đổi)".
  - `notify_lark_webhook_url` mang token gửi tin Lark → cũng che, không bày ra trang mà trưởng kho đọc được.
- **Sửa:**
  - `src/lib/audit-view/presenter.ts` (mới): dịch một dòng audit thành câu tiếng Việt — "Nguyễn A đã xoá camera “Cổng sau”". Phủ 52 action, nhãn trường, nhãn vai trò/trạng thái, bảng đổi-từ-gì-sang-gì, mức độ và nhóm việc để lọc. Action lạ (thêm sau) vẫn ra câu đọc được nhờ suy từ động từ cuối + loại đối tượng, **không** rơi ra mã trần. File thuần, không import React/server nên test chạy thẳng.
  - `src/lib/audit-view/resolve-names.ts` (mới): tra tên đối tượng (camera nào, ai, kho nào) theo lô — tối đa 7 truy vấn cho cả trang, không phải mỗi dòng một truy vấn. Đi qua đúng `scoped.select` mà route audit dùng, **không** dùng admin client trần: lộ tên đối tượng cross-tenant cũng là rò dữ liệu. Một bảng lỗi thì bỏ qua nhóm đó, các nhóm khác vẫn có tên — nhật ký không vì thế mà trắng trang.
  - **Cạnh kiểu dữ liệu:** `audit_logs.target_id` là `text` còn `id` các bảng đích là `uuid`. Một giá trị không phải UUID lọt vào `.in("id", …)` làm Postgres ném 22P02 và **cả nhóm** mất tên. Dữ liệu hiện tại sạch (đã kiểm), nhưng kiểu cột không chặn được route tương lai ghi bậy → lọc bằng regex trước khi truy vấn.
  - `src/app/api/audit/route.ts`: trả thêm `target_name` đã tra. Đối tượng đã xoá thì không tra ra — phía trình bày lùi về tên chụp trong metadata (`full_name`/`staff_code` mà route xoá đã lưu lại).
  - `src/app/dashboard/audit/page.tsx`: viết lại — gom theo ngày ("Hôm nay", "Hôm qua"), mỗi dòng một câu tiếng Việt, bảng đổi-từ-gì-sang-gì đã dịch tên trường, chấm màu theo mức độ, ô tìm kiếm + lọc theo mảng việc / mức độ. Mã kỹ thuật **không bị vứt đi** — nằm trong "Chi tiết kỹ thuật" gập lại, để khi điều tra sự cố vẫn tra được.
- **Files test:** `tests/audit-presenter.test.ts` (mới, 19 bài). Gồm bài canh: quét mọi lời gọi `audit()` trong `src/app/api` + `src/lib`, action nào chưa khai nhãn thì **đỏ kèm tên action** — thêm route mới mà quên khai thì bắt tại chỗ, không để người dùng phát hiện hộ. Bốn bài lấy thẳng từ dữ liệu thật (JSON lớn, token webhook, trường cấu hình tổ chức, action lịch sử).
- **Kết quả kiểm tra:** `pnpm test` **598/598** đạt (không hồi quy), `tsc` app và tests đều đạt, `eslint` 0 lỗi (1 cảnh báo `set-state-in-effect` là mẫu fetch-lúc-mở-trang sẵn có của repo — 29 chỗ khác trên dashboard cùng cảnh báo này). `next build` production đạt, `/dashboard/audit` có trong bản dựng. Đối chiếu tên tra được trên database thật: `Cam QR01`, `Hikvision Bàn 3 - Toàn cảnh` tra ra đúng; tài khoản đã xoá tra không ra và lùi đúng về `full_name` trong metadata.
- **Chưa làm:** trang tổng hợp audit các org ở platform (hiện phải impersonate từng org). Chưa chạy thử bằng mắt trên trình duyệt với tài khoản thật — cổng 3000 đang có dev server của chủ dự án nên chỉ dựng bản production để xác nhận trang biên dịch được.
- **Trạng thái:** Hoàn tất phần viết lại trang kho.

### [NHATKY-SANG-PLATFORM] - Chuyển hai trang nhật ký sang platform, thêm dropdown chọn kho

- **Chủ dự án chốt 25/09/2026:** đẩy cả hai trang sang platform, Nhật ký hệ thống có dropdown chọn từng kho. Ba điểm hỏi lại và câu trả lời: (1) **tách hai trang riêng** chứ không gộp với "Nhật ký kiểm toán" sẵn có; (2) dropdown **mặc định "Tất cả các kho"**; (3) **chuyển hẳn** — bỏ khỏi menu kho, không nhân đôi.
- **Vì sao tách chứ không gộp:** "Nhật ký kiểm toán" (`/platform/audit`) đọc `platform_audit_log` — thao tác CỦA quản trị nền tảng (impersonate, tạo/khoá tổ chức). Trang mới đọc `audit_logs` — việc người TRONG kho làm. Hai bảng khác cấu trúc, khác bộ lọc; gộp thì một trang phải ẩn/hiện cột theo chế độ.
- **Trang mới:**
  - `src/app/platform/org-audit/page.tsx` + `src/app/api/platform/org-audit/route.ts` (mới): nhật ký các kho, dropdown chọn kho, mặc định xem tất cả. Ở chế độ tất cả mỗi dòng gắn thẻ tên kho; chọn một kho thì bỏ thẻ cho đỡ rối.
  - `src/app/platform/changelog/page.tsx` (mới): khung platform bọc quanh dòng thời gian cập nhật.
  - `src/components/changelog/ChangelogTimeline.tsx` (mới): tách phần thân trang changelog ra dùng chung — nội dung sinh từ `changelog/` lúc build, không phụ thuộc tổ chức nên chỉ khung ngoài khác nhau.
- **Lọc theo kho chạy ở TRUY VẤN, không ở trình duyệt:** lọc phía client vẫn kéo dữ liệu mọi kho xuống máy người dùng, và làm hỏng ý nghĩa của giới hạn 200 dòng — 200 dòng mới nhất của MỌI kho, lọc xong còn vài dòng của kho cần xem. Đổi kho là gọi lại máy chủ (`load` phụ thuộc `orgId`).
- **Chỗ nhạy cảm — route này dùng admin client:** platform admin không có `organization_id` trong JWT nên RLS của `audit_logs` trả 0 dòng (cọc A3). Xem xuyên kho đúng là việc của trang, nhưng vì thế cổng chặn phải nằm ngay đầu route: `requirePlatformRole` gọi **trước** `createAdminClient()`, và có bài test canh đúng thứ tự đó. Thêm `resolveTargetNamesAdmin` tách riêng khỏi `resolveTargetNames` (bản tenant vẫn đi qua `scoped.select`, không đổi) — để không ai lỡ dùng bản bỏ-RLS ở route tenant.
- **Gỡ khỏi menu kho:** xoá `src/app/dashboard/audit/` và `src/app/dashboard/settings/changelog/`, gỡ mục trong `src/lib/nav.ts` và **gỡ cả entry trong `src/lib/nav-access.ts`** — `canSeeHref` trả false cho href chưa khai nên vừa mất mục menu vừa chặn người gõ thẳng đường dẫn.
- **Files test:** `tests/platform-org-audit-page.test.ts` (mới, 6 bài — canh trang cũ không sống lại, hai nguồn nhật ký không lẫn bảng, lọc không tụt xuống client, thứ tự guard/admin client); cập nhật `tests/changelog-page.test.ts` và `tests/role-permissions.test.ts` theo vị trí mới.
  - Hai bài đầu tiên **đỏ thật** vì cách kiểm quá thô — `includes("platform_audit_log")` bắt trúng cả tên bảng nằm trong chú thích giải thích. Sửa thành kiểm lời gọi thật (`from("platform_audit_log")`, `href: "..."`, `["/dashboard/..."`) chứ không kiểm chú thích.
- **Kết quả kiểm tra:** `pnpm test` **608/608** đạt, `tsc` app + tests đạt, `eslint` 0 lỗi (1 cảnh báo `set-state-in-effect` — mẫu fetch-lúc-mở-trang sẵn có, 29 chỗ khác cùng loại). `next build` đạt: bản dựng **có** `/platform/org-audit` + `/platform/changelog`, **không còn** `/dashboard/audit` + `/dashboard/settings/changelog`. Ba script guard prebuild (`check-write-routes`, `check-client-writes`, `check-signed-url`) đều xanh.
- **Đo cách ly trên database thật (cả hai vế):** hệ có 2 tổ chức — Betacom 352 việc, Đại Kim 76 việc. **Vế dương:** chọn Đại Kim ra đúng 76 dòng. **Vế âm:** trong kết quả đó, số dòng thuộc tổ chức khác = **0**. Chế độ "Tất cả": 200 dòng gần nhất đều tra ra tên kho, **0 dòng không ra tên**.
- **Còn lại:** `/api/audit` (route tenant cũ) giờ không còn ai gọi sau khi trang kho bị gỡ — giữ nguyên chứ chưa xoá, vì xoá bề mặt API là việc nên chủ dự án quyết. Nếu chốt bỏ hẳn thì xoá `src/app/api/audit/route.ts`; hàm `resolveTargetNames` (bản tenant) khi đó cũng thành không dùng.
- **Chưa làm:** chưa mở bằng mắt trên trình duyệt với tài khoản platform thật — cổng 3000 đang có dev server của chủ dự án nên chỉ dựng bản production để xác nhận trang biên dịch và có trong bản dựng.
- **Trạng thái:** Hoàn tất.

### [NHATKY-DOI-TEN-VA-DON-API] - Đổi tên "Nhật ký phiên bản" và gỡ route tenant cũ

- **Đổi tên (chủ dự án chốt 25/09/2026):** "Nhật ký cập nhật phiên bản" → **"Nhật ký phiên bản"**. Sửa nhãn menu (`src/lib/platform-nav.ts`), tiêu đề trang (`src/app/platform/changelog/page.tsx`), các chú thích trong `src/lib/changelog/*`, và bài test bám nhãn.
- **`changelog/README.md`:** sửa luôn chỗ chỉ sai đường — vẫn ghi "Quản lý hệ thống → …" từ trước khi chuyển sang platform. Nay ghi "Platform → Nhật ký phiên bản".
- **Giữ nguyên `changelog/2026-09-24.md`:** file đó là bản ghi lịch sử của ngày 24/09, lúc ấy trang thật sự mang tên cũ và thật sự nằm ở Quản lý hệ thống — viết lại là sửa quá khứ. Đã kiểm: hai dòng đó nằm NGOÀI mục `## Phát hành cho người dùng` nên không lọt ra giao diện (`grep` trong `generated.ts` = 0).
- **Gỡ `/api/audit` (chủ dự án chốt sau khi hỏi "xoá có ảnh hưởng gì không"):**
  - **Vì sao không ảnh hưởng — đã kiểm ba tầng:** (1) không có lời gọi tĩnh nào; (2) không có ghép chuỗi động nào dựng ra được đường dẫn đó (`"/api/" +`, `` `/api/${` `` đều 0 match — theo cọc "grep marker không đủ kết luận dead code"); (3) agent không gọi (chỗ `p.audit` trong `warehouse-agent` là field payload clip, khác hẳn), middleware không có whitelist riêng.
  - **Không mất dữ liệu:** route chỉ ĐỌC `audit_logs`. Bảng vẫn nguyên, nhật ký vẫn được ghi như cũ. Xoá route là bỏ một cửa sổ, không đụng vào kho.
  - **Không mất chức năng:** `/api/platform/org-audit` làm mọi thứ route này từng làm và nhiều hơn (xuyên kho + dropdown).
  - **Lý do xoá chứ không giữ:** nó là cửa có khoá (`requirePermission("audit.view")`) nhưng không trang nào dùng — sau này sửa phân quyền sẽ không ai nhớ kiểm nó. Bề mặt API không dùng là nợ, không phải dự phòng.
  - Gỡ luôn `resolveTargetNames` bản tenant (chỉ route vừa xoá dùng) và import `ScopedClient` thành thừa. Sửa phần mô tả đầu `resolve-names.ts` vì nó đang ghi ngược với cơ chế thật sau khi gỡ — file giờ chỉ còn bản admin cho platform, nên chú thích phải nói đúng điều đó kèm ràng buộc `requirePlatformRole` đứng trước `createAdminClient()`.
- **Files test:** cập nhật `tests/changelog-page.test.ts` (nhãn mới); thêm 2 bài vào `tests/platform-org-audit-page.test.ts` — canh route cũ không sống lại, và canh bản tenant của resolver đã gỡ (còn sót là dấu hiệu ai đó khôi phục nửa vời và có thể đang gọi admin client từ route tenant).
- **Kết quả kiểm tra:** `pnpm test` **609/609** đạt, `tsc` app + tests đạt, `eslint` 0 lỗi. `next build` đạt — bản dựng **không còn** `/api/audit` và `/dashboard/audit`, **có** `/api/platform/org-audit` + `/platform/org-audit` + `/platform/changelog`.
- **Nếu sau này muốn trả trang nhật ký về cho chủ kho:** dựng lại nhanh vì logic nhãn đã nằm sẵn ở `src/lib/audit-view/presenter.ts` dùng chung; chỉ cần viết lại route đi qua `scoped.select` (RLS đỡ) — KHÔNG dùng `resolveTargetNamesAdmin`.
- **Trạng thái:** Hoàn tất.
