-- ============================================================================
-- Chuỗi không có dáng mã vận đơn thì KHÔNG tạo đơn (sự cố kho Đại Kim
-- 25/09/2026)
--
-- Nhãn TikTok in HAI mã QR: mã vận đơn (TTVN…) và một mã link tới trang
-- shop (https://m.tiktok.shop/s/…). Camera bắt trúng cái link, và hệ thống
-- nhận bừa nó làm mã vận đơn:
--
--   27/08 01:53  valid           → tạo một "đơn đi" mã là cả cái URL
--   27/08 02:40  duplicated
--   25/09 (×8)   return_suspect  → lưới an toàn thấy "mã đã gửi đi bị quét
--                                  lại" nên ghi thành HÀNG HOÀN
--
-- Tức là một lượt đọc nhầm hồi tháng 8 đẻ ra 8 kiện hoàn ma trong một buổi
-- sáng tháng 9, lẫn vào nhật ký đóng hàng. Không ai giơ mã vận đơn nào cả.
--
-- Luật chặn, cố tình để RỘNG (thà nhận nhầm còn hơn chặn nhầm đơn thật):
--   - dài 8–40 ký tự
--   - chỉ gồm chữ, số, và - _ .
--   - bắt đầu bằng chữ hoặc số
--
-- Mọi mã thật đang dùng đều qua: 862487244176, 260924UUVC54EF,
-- SPXVN060122245929, TTVN1111790805. Còn URL thì rớt ngay vì có "://".
--
-- Chuỗi rớt luật này thành `invalid_code`: hiện ở "Cần xử lý" để người
-- vận hành biết camera đang đọc nhầm thứ gì, và KHÔNG tạo đơn, KHÔNG kích
-- hoạt lưới an toàn hàng hoàn.
--
...
CREATE OR REPLACE FUNCTION public.is_waybill_like(p_value text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
...
AS $fn$
  SELECT p_value IS NOT NULL
     AND length(p_value) BETWEEN 8 AND 40
     AND p_value ~ '^[A-Z0-9][A-Z0-9._-]*$';
$fn$;

COMMENT ON FUNCTION public.is_waybill_like(text) IS
  'Chuỗi quét có dáng mã vận đơn không. Dùng để loại URL / rác trước khi tạo đơn (sự cố 25/09/2026).';
