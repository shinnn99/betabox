-- DAN TOAN BO FILE NAY VAO SUPABASE STUDIO -> SQL EDITOR -> RUN
-- Gom: (1) ap migration 20260917090000, (2) ghi lich su migration, (3) kiem tra.
-- Phan (1) tu boc BEGIN/COMMIT: loi o dau thi khong co gi duoc ap.
...
-- ========== (2) LICH SU MIGRATION ==========
-- Khong ghi thi `supabase db push` sau nay se chay lai file 20260914080000
-- va cai lai trigger hong (lam hong moi lan quet ma van don).
-- Dung `where not exists` thay vi `on conflict`: chay dung ke ca khi cot
-- version khong co rang buoc duy nhat, va chay lai nhieu lan van an toan.
insert into supabase_migrations.schema_migrations (version, name, created_by)
select v.version, v.name, v.created_by
from (values
...
) as v(version, name, created_by)
where not exists (
  select 1 from supabase_migrations.schema_migrations m where m.version = v.version
);

-- ========== (3) KIEM TRA — bon dong deu phai bang 1 ==========
select 'trigger ghi theo ca' as muc, count(*)::int as ket_qua
  from pg_trigger where tgname = 'staff_work_sessions_recording_commands'
union all select 'trigger chup camera QR', count(*)::int
  from pg_trigger where tgname = 'packing_events_capture_qr_camera'
union all select 'cot proof_qr_camera_id', count(*)::int
  from information_schema.columns where table_name = 'packing_events' and column_name = 'proof_qr_camera_id'
union all select 'RLS order_proof_requests', relrowsecurity::int
  from pg_class where oid = 'public.order_proof_requests'::regclass;
